...
class Leaderboard:
    ...
    def create_leaderboard_frame(self):
        ...
        for user, score in self.top5.items():
            ...
            text_label.grid(row = num+1, column = 1,columnspan=2, pady=5)
            num += 1
        ...
        user_label.grid(row = 7, column = 1,columnspan=2, pady=10)
        ...
        #a retry button
        retry_button = Button(frame, text = "Retry", font = self.button_font, 
                             height=1, width=15, relief=RIDGE, bg="#fff3e6",
                             activebackground="#ceb79b",
                             command=self.retry)
        retry_button.grid(row=8, column=2, pady=10)        
