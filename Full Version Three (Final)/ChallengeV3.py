...
class Challenge:
    ...
    def __init__(self, file_path, turn):
        ...
        self.math_a = []
        self.num6 = 0    
        self.help_button_image_file_path = r"N:\13PRG\st21121-Pika\Assessments\91906PikaRanzinger\Challenge_Component\help.png"
        self.challenge_help_image_file_path = r"N:\13PRG\st21121-Pika\Assessments\91906PikaRanzinger\Challenge_Component\challenge_help.png"
        self.alive = True 
        ...
        help_button.grid(row=0, column=0, pady=10, padx=(850,0))
    ...
    def help_window(self):
        ...
        #the image
        self.help_window_pic = PhotoImage(file=self.challenge_help_image_file_path)
    ...
    def create_challenge6_1(self):
        ...
        challenge_label = Label(frame, text= "", font=self.text_font, 
                                justify="left", bg="#ffe6c9")
        challenge_label.grid(row=3, column=0, columnspan=4, pady=20, padx=(200,0))
        
        #checker label
        checker_label = Label(frame, text="", bg="#ffe6c9", font=self.error_font)
        checker_label.grid(row=4, column=0, columnspan=4, padx=(200,0))
    ...
    def create_challenge6_2(self):
        ...
        #answer label and entry
        answer_label = Label(frame, text="Enter Number:", bg="#ffe6c9", font=self.label_font)
        answer_label.grid(row=5, column=0,pady=20, padx=(350,0), sticky="e")
        
        answer_entry = Entry(frame, bg="#ffe6c9", font=self.label_font, width=10)
        answer_entry.grid(row=5, column=1, sticky="w",pady=20, padx=(0,150))  
    ...
    def create_challenge7(self):
        ...
        challenge_label = Label(frame, text= "", font=self.text_font, 
                                justify="left", bg="#ffe6c9")
        challenge_label.grid(row=1, column=0, columnspan=2, pady=20)
        
        #checker label
        checker_label = Label(frame, text="", bg="#ffe6c9", font=self.error_font)
        checker_label.grid(row=2, column=0, columnspan=2)
        ...
        for i in range(4):
            choice_button = Button(frame, text="Confirm Choice",height=2, width=30, font = self.button_font, 
                                    relief=RIDGE, bg="#fff3e6", activebackground="#ceb79b",
        ...
        start_button.grid(row=6, column=0, columnspan=2, sticky="we", pady=20, padx=20)        
    ...
    def challenge1_2_check(self, answer_text, checker):
        ...
        for items in answer:
            ...
            try:
                #checking that answer is number
                items = int(items)
                answers.append(items)
                if len(answers) == 5:
                    #there are 5 and they are numbers 
                    entry = True
            except ValueError:
                #else add attempt and error message 
                self.attempts += 1 
                checker.config(text="Enter 5 numbers")
    ...
    def challenge3(self, label, answer_text, button, checker_label, answer_label):
        ...
        if answer.strip() != "":
            #checking if it's empty as a cont button
            try: 
                #checking that the answer is a number
                answer = int(answer)
                entry = True
            except ValueError:
                #else not allow and adds an attempt 
                self.attempts += 1 
                checker_label.config(text="Enter a number")
        else:
            entry = True
        ...
        if self.round_checker == 1:
            ...
            elif answer in round1: 
                #if the answer is wrong and between 1 to 3  
                self.attempts += 1 
...
                """)
                ...
                #if number is out of range of options 
                self.attempts += 1
                checker_label.config(text="Enter a number between 1 and 3")
...
                """)
                ...
                #if number is out of range of options 
                self.attempts += 1
                checker_label.config(text="Enter a number between 1 and 5")            
...
                """)     
                ...
                #if number is out of range of options 
                self.attempts += 1
                checker_label.config(text="Enter a number between 1 and 7")            
    ...
    def challenge6_1(self, label, answer_text):
        ...
        try:
            ...
            if user <=100 and user > 0:
                ...
                self.attempts += 1
                label.config(text="Enter a number from 1 to 100")
        except ValueError:
            self.attempts += 1
            label.config(text="Enter a number from 1 to 100")
        
        if entry is True:
            #if its a number between 1 to 100
    ...
    def challenge6_2_start(self, button, answer_label, answer_entry):
        ...
        self.math_q = random.sample(list(maths.keys()), self.MAX_Q)
    ...
    def challenge6_2_check(self, label, correct, answer_text, start_button):
        ...
        try:
            ...
            self.attempts += 1 
            correct.config(text="Enter a number")
        
        if entry is True:
            if self.num6 == self.MAX_Q:
    ...
    def challenge7_start(self, button_lists, start_button, label):
        ...
        for btn in button_lists:
            ...
            row_num = i//2 
            col_num = i%2
            btn.grid(row=(row_num+3), column=col_num, padx=20, pady=20, sticky="we")
    ...
    def challenge8_start(self, button, answer_label, answer_entry):
        ...
        answer_label.grid(row=5, column=1,columnspan=2,pady=20, padx=(100))
        answer_entry.grid(row=6, column=1, columnspan=2,pady=20, padx=(100))        
...
files = {"CHALLENGE_FILE": r"N:\13PRG\st21121-Pika\Assessments\91906PikaRanzinger\Challenge_Component\chalenge.json"}
...
'''
entry = validate_file(files["CHALLENGE_FILE"])

if entry is True: 
    chal = Challenge(CHALLENGE_FILE_PATH, turn)
    frame = chal.find_frame(NUMBERS[8])
